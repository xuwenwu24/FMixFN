...
cat train_tmp.raw | wc | awk '{print $1}' > parameter.txt #the number of training individuals
cat train_tmp.raw | awk '{print NF-6}' | sed -n '1p' >> parameter.txt #add the number of markers
echo 1 >> parameter.txt #add the number of fixed effects, Suppose the number of fixed effects is 1
echo 0.25 >> parameter.txt #add PI value
echo 0.61 >> parameter.txt #add the heritability value, Suppose the heritability value is 0.61
cat validation_tmp.raw | wc | awk '{print $1}'  >> parameter.txt #add the number of markers
echo 1 >> parameter.txt #Whether calculate the predictive ability (input 1 means to calculate, input 0 means not calculated)

##Start running FMixFN with all files ready
##Run
./FMixFN
